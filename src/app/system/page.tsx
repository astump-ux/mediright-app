...
/**
 * /system — Admin-only page for editing KI-Prompts and WhatsApp templates.
 * Reuses the same editor UI as /admin but scoped to 'prompts' and 'nachrichten' categories.
...
const SYSTEM_CATEGORIES = ['prompts', 'nachrichten']
...
export default function SystemPage() {
  ...
  const fetchSettings = useCallback(async () => {
    ...
    if (res.ok) {
      ...
      // Filter to system-only categories
      const filtered = data.filter(s => SYSTEM_CATEGORIES.includes(s.category))
      setSettings(filtered)
      const initial: Record<string, string> = {}
      filtered.forEach(s => { initial[s.key] = s.value })
      setEdits(initial)
    } else {
  ...
  useEffect(() => {
    if (roleChecked) fetchSettings()
  }, [roleChecked, fetchSettings])
      ...
      <main style={{ padding: '32px 24px', maxWidth: 900, margin: '0 auto', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
        ...
        <div style={{ marginBottom: 32 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
            <h1 style={{ fontFamily: "'DM Serif Display', Georgia, serif", fontSize: 28, color: navy, margin: 0, fontWeight: 400 }}>
              System
            </h1>
            <span style={{
          ...
          <p style={{ color: slate, fontSize: 14, margin: 0 }}>
            KI-Prompts und WhatsApp-Nachrichten verwalten. Änderungen sind sofort aktiv.
          </p>
        </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  {items.map(setting => (
                    <div key={setting.key} style={{
                      ...
                      border: isDirty(setting.key) ? `1.5px solid ${mint}` : '1.5px solid transparent',
                    }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 10, gap: 8, alignItems: 'center' }}>
                        ...
                        <button
                          ...
                          style={{
                            ...
                            cursor: isDirty(setting.key) ? 'pointer' : 'not-allowed',
        ...
        {settings.length === 0 && !error && (
          <div style={{ textAlign: 'center', color: slate, fontSize: 14, padding: '48px 0' }}>
            Keine System-Einstellungen gefunden.
          </div>
        )}
      </main>
    </>
